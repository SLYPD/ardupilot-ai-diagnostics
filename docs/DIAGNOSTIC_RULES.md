...
When evaluating motor capabilities, ESC sync issues, or thrust limitations, all calculations and conclusions **must refer to the maximum physical output capability of the motor.**
* **DO NOT** base thrust limits or safety warnings on the take-off point or hover throttle. 
...
## 4. MOTOR IMBALANCE DETECTION
* A healthy hexacopter maintains motor PWM outputs within ~15% of each other during stable flight.
* If opposing motor pairs show sustained divergence (e.g., C1 at 1300 while C4 is pinned at 1940), the flight controller is compensating for a physical or aerodynamic failure on the overloaded motor's arm.
* This pattern, combined with elevated vibration on the matching axis, confirms a propulsion system fault.

## 5. THE INCONCLUSIVE MANDATE
